...
def evaluate(cultures:str,space:str) -> float:
	"""
	Returns an inverted normalised Levenshtein distance for strings of cultures and search spaces
	"""
	return 1 - ld(cultures,space) / max([len(cultures),len(space)])
...
class Society:
	"""
	Represents a society with cultural systems and search spaces that culturally evolve over time.
	...
	- A cultural system (bitstring) representing the interdependent repertoire of techniques, behaviours, artifacts and knowledge available to a society
	- Resource management including initial endowment and depletion tracking
	
	The society can change both their cultural systems and search spaces, with resource allocation determining the balance between these two evolutionary processes.
	"""
	def __init__(self, s_length:int, s_prob:float, c_length:int, c_prob:float, initial_endowment:float):
		if s_prob == None:
			s_prob = np.random.uniform(low=0.0,high=1.0,size=None)
		if c_prob == None:
			c_prob = np.random.uniform(low=0.0,high=1.0,size=None)
		# Generating initial search space
		self.space = string_generator(p=s_prob,l=s_length)
		# Generating initial cultural system
		self.cultures = string_generator(p=c_prob,l=c_length)
	...
	def tradeoff(self,iterations:int,p:float) -> list:
		...
		If True: change cultural system
		If False: change search space
		"""
		return np.random.choice(a=[True,False],size=iterations,p=[p,1-p])

	def ce_cultures(self, cultures:str, space:str, η:float) -> str:
		"""
		Process for the evolution of cultural systems
		Changes cultural system either via stochastic or deterministic process
		The probability of a stochastic or deterministic process is controlled by η
		"""
		new_tech = string_edit(str_1=cultures, str_2=space, eff=η)
		return new_tech

	def ce_space(self, space:str, cultures:str, λ:float) -> str:
		"""
		Process for the evolution of search spaces
		...
		"""
		new_space = string_edit(str_1=space, str_2=cultures, eff=λ)
		return new_space

def simulation(seed:int, path:str, printing:bool, write:bool, limit:int, generations:int, s_length:int, s_prob:float, c_length:int, c_prob:float, η:float, λ:float, initial_endowment:float, p_tradeoff:float) -> pd.DataFrame:
	"""
	Simulation function for the evolution of cultural systems and search spaces.
	...
	limit : int
		Upper-limit on cultural complexity (terminates if matched or exceeded)
	generations : int
	...
	c_length : int
		Initial length of cultural system bitstring
	c_prob : float
	...
	p_tradeoff : float
		Probability of allocating resources to changing cultural system or changing search space
	...
	3. Cultural complexity limit exceeded
	"""
	...
	s = Society(s_length=s_length, s_prob=s_prob, c_length=c_length, c_prob=c_prob, initial_endowment=initial_endowment)

	# Generation 0
	# Calculate initial effectiveness
	effectiveness = evaluate(s.cultures, s.space)
	# Get net resources
	net_resources = resource_function(cS=len(s.space), effectiveness=evaluate(s.space, s.cultures), cT=len(s.cultures))
	# Resources available to a society
	available_resources = s.update_resource_store(net_resources)
	# Append to output
	output.append([seed, 0, len(s.cultures), len(s.space), effectiveness, c_length, s_length, η, λ, initial_endowment, p_tradeoff, available_resources, s.resource_store])
	...
	for gen in range(1, generations):
		...
		if s.is_depleted:
			print(f'{seed} depleted resource store at generation {gen}. Final state: tech={len(s.cultures)}, space={len(s.space)}')
			# Generate dataframe
		...
		if iterations > 0:
			# Get list of evolutionary events: True (change cultural system), False (change search space)
			choices = s.tradeoff(iterations=iterations, p=p_tradeoff)
			# For each evolutionary event, make a change to either the cultural system or search space
			for choice in choices:
				# Make change to the cultural system
				if choice == True:
					s.cultures = s.ce_cultures(cultures=s.cultures, space=s.space, η=η)
				# Make change to the search space
				else:
					s.space = s.ce_space(space=s.space, cultures=s.cultures, λ=λ)
		# Calculate effectiveness
		effectiveness = evaluate(s.cultures, s.space)
		# Append to output
		output.append([seed, gen, len(s.cultures), len(s.space), effectiveness, c_length, s_length, η, λ, initial_endowment, p_tradeoff, available_resources, s.resource_store])
		# Print seed, generation, cultural complexity, search space complexity, available resources and effectiveness
		if printing == True:
			print(f'Seed: {seed}   Generation: {gen}   Culture Complexity: {len(s.cultures)}   Space Complexity: {len(s.space)}  Resources: {available_resources:.2f}   Store: {s.resource_store:.2f}   Effectiveness: {effectiveness:.3f}')
		# Check for complexity upper-limit (if True, end simulation)
		if len(s.cultures) >= limit:
			print(f'{seed} reached cultural complexity limit at generation {gen}')
			df = dataframe(data=output)
			if write:
				writing(df=df, path=path, seed=seed)
			return df
...
for seed in seeds:
	simulation(
		...
		limit=10000, # Upper-limit of cultural complexity for simulation 
		...
		c_length=2, # Initial length of cultural system
		c_prob=None, # Initial probability of 0s and 1s for cultural system (default: None)
		η=0.95, # Eta: Controls probability for type of change to cultural systems. Maximally stochastic (η=0.0), Maximally deterministic (η=1.0)
		λ=0.95, # Lambda: Controls probability for type of change to search spaces. Maximally stochastic (λ=0.0), Maximally deterministic (λ=1.0)
